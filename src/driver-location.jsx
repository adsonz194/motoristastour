...
export function DriverLocationSharingCard({ data, user, token, notify }) {
  ...
  useEffect(() => {
    const handleRequestedStart = () => { void startSharing(); };
    window.addEventListener('motoristastour:start-driver-location', handleRequestedStart);
    return () => window.removeEventListener('motoristastour:start-driver-location', handleRequestedStart);
  }, [attendanceForToday?.id, driverCannotShare, state, sharingWindow.open, locationTestModeForDriver.active]);
  useEffect(() => {
...
export function ConsultantSupportLocationPanel({ requestId, accessToken, initialRequest = null, onRequestChange, onUnavailable }) {
  ...
  return <section className="panel driver-map-panel consultant-support-map">
    ...
    {request?.tourId && <div className="consultant-route-summary"><strong>{request.tourLabel || 'Tour'} · {request.requestedByName || request.consultantName || request.selfGenName}</strong><span><MapPin size={14} /> {request.guestLocationLabel || request.routeStageLabel}{request.destinationName ? ` → ${request.destinationName}` : ''}</span></div>}
    <LocationTestModeNotice testMode={locationTestMode} />
