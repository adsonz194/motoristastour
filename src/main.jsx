...
function TourTable({ tours, data, onAction, compact = false, empty = 'Nenhum grupo nesta etapa.' }) {
  ...
  return <div className={classNames('table-wrap', 'tour-table', compact && 'table-compact')}><table><thead><tr><th>Consultor</th><th>Família / Casal</th><th>Pessoas</th><th>Carrinhos</th><th>Motoristas</th><th>Status</th><th aria-label="Ações" /></tr></thead><tbody>{tours.map((tour) => {
    ...
    return <tr key={tour.id}><td><div className="name-cell"><Avatar name={consultantName} color="pink" /><span>{consultantName}</span></div></td><td><strong>{tour.groupName}</strong><small className="schedule-info">{WAVES[tour.wave]?.label || 'Ola não definida'} · {tour.scheduledTime || '—'}</small>{team && <small className="tour-team">{team}</small>}{tour.selfGuide && <small className="self-guide">Self Gean</small>}{tour.status === 'NA_CASA' && <div className="home-presence"><House size={14} /><span><b>NA CASA COM O CASAL:</b> {driverInfo.active}</span></div>}{tour.status === 'NA_CASA' && driverInfo.returned !== '—' && <small className="returned-driver">VOLTOU AO PRESTIGE: {driverInfo.returned}</small>}</td><td>{tour.people || '—'}</td><td>{driverInfo.cartsAtHome || '—'}</td><td><strong>{driverInfo.active}</strong>{driverInfo.returned && driverInfo.returned !== '—' && <small className="schedule-info">Retornou ao Prestige: {driverInfo.returned}</small>}</td><td><StatusPill status={tour.status} /></td><td className="actions-cell">{isOnInitialRoute && <button className="mini-action secondary" onClick={() => onAction(tour, 'deliver-gallery')}>Chegou direto à Galeria</button>}{canCorrectToHome && <button className="mini-action secondary" onClick={() => onAction(tour, 'correct-to-home')}>{actionMeta['correct-to-home'].label}</button>}{tour.status === 'NA_CASA' && <button className="mini-action secondary" onClick={() => onAction(tour, 'return-prestige')}>Trocar motoristas</button>}{canChangeDestination && <button className="mini-action secondary" onClick={() => onAction(tour, 'change-destination')}>{actionMeta['change-destination'].label}</button>}{action && <button className="mini-action" onClick={() => onAction(tour, action)}>{actionMeta[action].label}</button>}</td></tr>;
  })}</tbody></table></div>;
}
