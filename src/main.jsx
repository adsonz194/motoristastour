...
const actionMeta = {
  start: { title: 'Iniciar tour', text: 'Defina os carrinhos, hóspedes e motoristas para registrar a saída do Prestige.', label: 'Iniciar tour', allocations: true },
...
function TransferTable({ transfers, onAction, user, empty = 'Nenhum convite agendado para hoje.' }) {
  ...
  return <div className="table-wrap transfer-table"><table><thead><tr><th>Horário</th><th>Onda do tour</th><th>Grupo / Convidados</th><th>Pessoas</th><th>Concierge</th><th>Trajeto</th><th>Status</th><th aria-label="Ações" /></tr></thead><tbody>{transfers.map((transfer) => {
    const next = user?.role === 'CONCIERGE' ? null : transfer.status === 'AGENDADO' ? 'start' : transfer.status === 'EM_DESLOCAMENTO' ? 'arrive' : null;
...
function Dashboard({ data, user, token, refresh, notify, onAction, onCreate, onCreateTransfer, onTransferAction, setPage }) {
  ...
  const checkIn = ['MOTORISTA', 'HOSTESS'].includes(user.role) && <CheckInCard data={data} user={user} token={token} refresh={refresh} notify={notify} />;
...
function HostessTourModal({ onClose, token, refresh, notify }) {
  ...
  const [selfGuideQuantity, setSelfGuideQuantity] = useState('0');
  const [wave, setWave] = useState('WAVE_1');
  const [saving, setSaving] = useState(false);
  async function submit(event) {
    event.preventDefault(); setSaving(true);
    try { await api(token, '/api/tours/hostess', { method: 'POST', body: JSON.stringify({ quantity: Number(quantity), selfGuideQuantity: Number(selfGuideQuantity), wave }) }); await refresh(); notify('Quantidade de tours registrada.', 'success'); onClose(); } catch (error) { notify(error.message, 'error'); } finally { setSaving(false); }
  }
  return <Modal title="Registrar quantidade de tours" onClose={onClose}><form className="modal-form" onSubmit={submit}><label>Quantidade de tours<input type="number" min="1" max="30" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label>Quantidade de tours Self Guide<input type="number" min="0" max={Math.max(0, Number(quantity) || 0)} value={selfGuideQuantity} onChange={(event) => setSelfGuideQuantity(event.target.value)} required /></label><label>Onda do tour<select value={wave} onChange={(event) => setWave(event.target.value)}>{Object.entries(WAVES).map(([key, item]) => <option value={key} key={key}>{item.label} · saída às {item.tourTime}</option>)}</select></label><div className="role-help">A Hostess registra somente as quantidades de tours, Self Guide e a onda. Nome da família, hóspedes, consultor, carrinhos e motoristas serão informados pelo motorista na saída.</div><button className="button button-primary" disabled={saving}>{saving && <LoaderCircle className="spin" size={17} />} Registrar tours</button></form></Modal>;
}
...
function HostessPrestigePage({ data, user, token, refresh, notify }) {
  ...
  const selfGuideToday = (data.tours || []).filter((tour) => tour.registeredBy === 'HOSTESS' && tour.selfGuide).length;
  return <><CheckInCard data={data} user={user} token={token} refresh={refresh} notify={notify} /><section className="page-title"><div><span>CONTROLE OPERACIONAL</span><h1>Prestige Praia do Forte</h1><p>A Hostess registra somente as quantidades de tours, Self Guide e a onda.</p></div><button className="button button-primary" onClick={() => setOpen(true)}><Plus size={18} /> Quantidade de tours</button></section><section className="hostess-grid"><article><Route size={35} /><div><span>TOURS REGISTRADOS</span><strong>{registeredToday}</strong><small>{awaitingDriver.length} aguardando identificação pelo motorista</small></div></article><article><Flag size={35} /><div><span>TOURS SELF GUIDE</span><strong>{selfGuideToday}</strong><small>registrados pela Hostess hoje</small></div></article></section><section className="panel hostess-note"><h2>Registro simplificado</h2><p>Nesta aba a Hostess não informa família, consultor, quantidade de hóspedes ou motoristas: somente a quantidade de tours, a quantidade Self Guide e a onda.</p></section>{open && <HostessTourModal onClose={() => setOpen(false)} token={token} refresh={refresh} notify={notify} />}</>;
}
...
function CreateTourModal({ data, onClose, token, refresh, notify }) {
  const [form, setForm] = useState({ quantity: '1', selfGuideQuantity: '0', wave: 'WAVE_1' });
  const [saving, setSaving] = useState(false);
  async function submit(event) {
    event.preventDefault(); setSaving(true);
    try { await api(token, '/api/tours', { method: 'POST', body: JSON.stringify({ ...form, quantity: Number(form.quantity), selfGuideQuantity: Number(form.selfGuideQuantity) }) }); await refresh(); notify('Quantidade de tours registrada.', 'success'); onClose(); } catch (error) { notify(error.message, 'error'); } finally { setSaving(false); }
  }
  return <Modal title="Cadastrar quantidade de tours" onClose={onClose}><form className="modal-form" onSubmit={submit}><label>Quantidade de tours<input type="number" min="1" max="30" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value })} required /></label><label>Quantidade de tours Self Guide<input type="number" min="0" max={Math.max(0, Number(form.quantity) || 0)} value={form.selfGuideQuantity} onChange={(event) => setForm({ ...form, selfGuideQuantity: event.target.value })} required /></label><label>Onda do tour<select value={form.wave} onChange={(event) => setForm({ ...form, wave: event.target.value })}>{Object.entries(WAVES).map(([key, wave]) => <option key={key} value={key}>{wave.label} · saída às {wave.tourTime}</option>)}</select></label><div className="role-help">Registre a quantidade total, a quantidade Self Guide e a onda. A família, hóspedes, consultor, carrinhos e motoristas serão informados pelo motorista no início do tour.</div><button className="button button-primary" disabled={saving}>{saving && <LoaderCircle className="spin" size={17} />} Registrar tours</button></form></Modal>;
}
...
function ActionModal({ tour, action, data, user, onClose, token, refresh, notify }) {
  ...
  const needsDetails = action === 'start' && (tour.requiresDetails || !tour.people);
  const guestCapacity = 4;
  const buildAllocations = (people) => {
    let remaining = Number(people) || 0; const rows = [];
    while (remaining > 0) { const seats = Math.min(guestCapacity, remaining); rows.push({ driverId: '', cartId: '', seats: String(seats) }); remaining -= seats; }
    return rows;
  };
  const [intake, setIntake] = useState({ groupName: needsDetails ? '' : tour.groupName, people: needsDetails ? '' : String(tour.people || ''), consultantId: tour.consultantId || '', selfGuide: Boolean(tour.selfGuide) });
  const [allocations, setAllocations] = useState(() => meta.allocations && !needsDetails ? buildAllocations(tour.people) : []);
  ...
  const guests = needsDetails ? Number(intake.people) || 0 : tour.people;
  const minimumCarts = guests ? Math.ceil(guests / guestCapacity) : 0;
  const allocatedGuests = allocations.reduce((sum, allocation) => sum + (Number(allocation.seats) || 0), 0);
  ...
  function setGuestQuantity(value) {
    setIntake({ ...intake, people: value });
    const needed = Math.ceil((Number(value) || 0) / guestCapacity);
    setAllocations((current) => { const next = [...current]; while (next.length < needed) { const filled = next.reduce((sum, item) => sum + (Number(item.seats) || 0), 0); next.push({ driverId: '', cartId: '', seats: String(Math.min(guestCapacity, Math.max(1, (Number(value) || 1) - filled))) }); } return next; });
  }
  async function submit(event) {
    event.preventDefault(); setSaving(true);
    const body = { action, allocations, destinationId, driverId: homeDriverId, homeDecision, ...intake };
  ...
  return <Modal title={meta.title} onClose={onClose}><div className="action-tour-summary"><Avatar name={tour.groupName} color="teal" /><div><strong>{needsDetails ? 'Tour aguardando identificação' : tour.groupName}</strong><span>{needsDetails ? 'Dados serão preenchidos pelo motorista' : `${tour.people} hóspedes · ${STATUS[tour.status]?.label}`}</span></div></div><p className="modal-intro">{meta.text}</p><form className="modal-form" onSubmit={submit}>{needsDetails && <div className="intake-form"><label>Família ou casal<input value={intake.groupName} onChange={(event) => setIntake({ ...intake, groupName: event.target.value })} placeholder="Ex.: Família de Maria" required /></label><label>Quantidade de hóspedes<input type="number" min="1" max="48" value={intake.people} onChange={(event) => setGuestQuantity(event.target.value)} required /></label><label>Consultor responsável (opcional)<select value={intake.consultantId} onChange={(event) => setIntake({ ...intake, consultantId: event.target.value })}><option value="">Sem consultor informado</option>{data.consultants.filter((item) => item.active).map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label className="checkbox-label"><input type="checkbox" checked={intake.selfGuide} onChange={(event) => setIntake({ ...intake, selfGuide: event.target.checked })} /> Grupo Self Guide</label></div>}{meta.allocations && <div className="allocation-form"><div className="allocation-title"><span>Alocação de transporte</span><small>{minimumCarts ? `Mínimo: ${minimumCarts} carrinho${minimumCarts > 1 ? 's' : ''} para ${guests} hóspedes` : 'Informe a quantidade de hóspedes'}</small></div><p className="allocation-help">Cada carrinho comporta 5 passageiros: 1 lugar do consultor e até 4 hóspedes. Distribua os hóspedes como precisar, por exemplo 3 e 2.</p>{allocations.map((allocation, index) => <div className="allocation-row allocation-row-guests" key={index}><label>Motorista<select value={allocation.driverId} onChange={(event) => updateAllocation(index, 'driverId', event.target.value)} required><option value="">Selecione</option>{drivers.map((driver) => <option value={driver.id} key={driver.id}>{driver.name}</option>)}</select></label><label>Carrinho<select value={allocation.cartId} onChange={(event) => updateAllocation(index, 'cartId', event.target.value)} required><option value="">Selecione</option>{carts.map((cart) => <option value={cart.id} key={cart.id}>{cart.name} · 4 hóspedes + consultor</option>)}</select></label><label>Hóspedes<input type="number" min="1" max="4" value={allocation.seats} onChange={(event) => updateAllocation(index, 'seats', event.target.value)} required /></label>{allocations.length > minimumCarts && <button type="button" className="remove-allocation" onClick={() => setAllocations((current) => current.filter((_, itemIndex) => itemIndex !== index))}>Remover</button>}</div>)}<div className="allocation-footer"><span>{allocatedGuests}/{guests || '—'} hóspedes distribuídos</span><button type="button" className="text-button" onClick={() => setAllocations((current) => [...current, { driverId: '', cartId: '', seats: '1' }])}><Plus size={15} /> Adicionar carrinho</button></div></div>}{meta.individualArrival && <div className="allocation-form"><div className="allocation-title"><span>Registro individual do motorista</span><small>{pendingArrivals.length} pendente{pendingArrivals.length === 1 ? '' : 's'}</small></div><label>Motorista<select value={homeDriverId} onChange={(event) => setHomeDriverId(event.target.value)} required>{pendingArrivals.map((item) => { const driver = data.drivers.find((candidate) => candidate.id === item.driverId); return <option value={item.driverId} key={item.driverId}>{driver?.name || 'Motorista'}</option>; })}</select></label><label>Ao chegar na Casa<select value={homeDecision} onChange={(event) => setHomeDecision(event.target.value)}><option value="AGUARDOU_NA_CASA">Permaneceu aguardando com a família</option><option value="DEIXOU_NA_CASA">Deixou a família e voltou ao Prestige</option></select></label></div>}{meta.destination && <label>Destino final<select value={destinationId} onChange={(event) => setDestinationId(event.target.value)} required>{data.destinations.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}<button className="button button-primary" disabled={saving || (meta.individualArrival && !homeDriverId)}>{saving && <LoaderCircle className="spin" size={17} />} <Check size={17} /> Confirmar</button></form></Modal>;
}
